#!/usr/bin/env python3
"""Generate 80 verified MCQ questions (orders 21-100) per quantitative topic."""
...
def _gen_number_system(order: int) -> dict:
...
TOPIC_GENERATORS = {
    "number-system": _gen_number_system,
}


def generate_topic(topic: str, group: str) -> dict:
    from generators.registry import ALL_GENERATORS

    gen = ALL_GENERATORS.get(topic)
    if not gen:
        raise ValueError(f"No generator for {topic}")
    existing = load_existing_texts(EXISTING / f"{topic}.json")
    questions = []
    seen = set(existing)
...
def main():
    ...
    topics = [t for t in audit if t["category"] == "quantitative"]
    ...
    for t in topics:
        slug = t["topic"]
        if slug not in TOPIC_GENERATORS:
            print(f"SKIP (no generator): {slug}")
            continue
        data = generate_topic(slug, t["group"])
        path = OUT / f"{slug}.json"
        path.write_text(json.dumps(data, indent=2, ensure_ascii=False), encoding="utf-8")
        print(f"Generated {len(data['questions'])} for {slug} -> {path}")
