"""Replace NODEJS_GUIDE in guideData.js with the comprehensive 20-phase version."""

NEW_GUIDE = r"""export const NODEJS_GUIDE = [
  {
    phase: '01',
...
content = open('C:/manmadha/Student-project/FrontEnd/src/pages/deployment/guideData.js', 'r', encoding='utf-8').read()

marker_start = '// ─── Node.js / Express → Render (comprehensive 20-phase guide) ──────────────\nexport const NODEJS_GUIDE = ['
marker_end = '\n// ─── MERN Stack → React/Vercel + Express/Render + MongoDB Atlas ───────────────'

old_start = content.index(marker_start)
old_end = content.index(marker_end, old_start)

new_content = content[:old_start] + '// ─── Node.js / Express → Render ────────────────────────────────────────────────\n' + NEW_GUIDE.strip() + content[old_end:]

with open('C:/manmadha/Student-project/FrontEnd/src/pages/deployment/guideData.js', 'w', encoding='utf-8') as f:
    f.write(new_content)

print('Done. Guide replaced successfully.')
