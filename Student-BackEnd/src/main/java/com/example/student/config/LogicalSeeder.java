...
/**
 * Seeds the logical-reasoning category from
 * {@code resources/seed/aptitude-logical.json}.
 *
 * Groups go into the shared {@code aptitude_groups} collection (navigation);
 * topics go into the dedicated {@code logical_topics} collection with the
 * purpose-built pattern-based lesson model.
 *
 * The lesson block is treated as JSON-source-of-truth (re-authoring overwrites
 * the DB whenever the JSON differs). Pruning is scoped to the logical category
 * only, so it never touches quantitative or verbal data.
 */
@Component
@Order(0)
public class LogicalSeeder {

    private static final Logger log = LoggerFactory.getLogger(LogicalSeeder.class);
    private static final String SEED_FILE = "seed/aptitude-logical.json";
    private static final String CATEGORY = "logical";

    private final LogicalTopicRepository topicRepo;
    private final AptitudeGroupRepository groupRepo;
    private final CacheService cacheService;
    private final ObjectMapper objectMapper = new ObjectMapper();

    public LogicalSeeder(LogicalTopicRepository topicRepo,
    ...
    static class SeedGroup {
        ...
        public String icon;
        public int order;
        public List<LogicalTopic> topics = new ArrayList<>();
    }
    ...
    public void seed() {
        try {
            ...
            if (taxonomy.isEmpty()) {
                log.info("Logical seeder: no data in {} — skipping (logical data left untouched).", SEED_FILE);
                return;
            }
            ...
            List<LogicalTopic> topicsToInsert = new ArrayList<>();
            ...
            for (SeedCategory cat : taxonomy) {
                ...
                for (SeedGroup g : cat.groups) {
                    ...
                    for (LogicalTopic def : g.topics) {
                        ...
                        Optional<LogicalTopic> tExistingOpt = topicRepo.findByTopic(def.getTopic());
            ...
            // Prune stale logical topics.
            List<LogicalTopic> orphanTopics = new ArrayList<>();
            for (LogicalTopic t : topicRepo.findAll()) {
                if (!keepTopics.contains(t.getTopic())) orphanTopics.add(t);
            }
            if (!orphanTopics.isEmpty()) topicRepo.deleteAll(orphanTopics);

            // Prune stale groups — scoped to the logical category only.
            ...
            log.info("Logical seeder: groups[{} new, {} upd, {} pruned] topics[{} new, {} upd, {} pruned].",
                    gInserted, gUpdated, orphanGroups.size(),
                    tInserted, tUpdated, orphanTopics.size());
        } catch (Exception e) {
            log.error("Logical seeder failed — app continues: {}", e.getMessage(), e);
        }
    }
    ...
    /** Keep DB topic in sync with the JSON. Lesson + videos are JSON-source-of-truth. */
    private boolean refreshTopic(LogicalTopic existing, LogicalTopic def) {
        boolean changed = false;
        if (!equalsSafe(existing.getCategory(), CATEGORY))                { existing.setCategory(CATEGORY); changed = true; }
        ...
        // Lesson: JSON wins. Only overwrite when the JSON provides one and it differs
        // (a null/absent JSON lesson never wipes existing content).
    ...
    private boolean jsonEquals(Object a, Object b) {
        try {
            ...
            return false; // treat as different → safe re-write
        }
    }
